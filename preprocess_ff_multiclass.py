...
def parse_args():
    ...
    parser.add_argument('-nt', '--num_frames_train', type=int, default=None,
...
def main(args):
    ...
    for use in USAGES:
        ...
        for i, mm in enumerate(MANIPULATION_METHODS[args.manipulation_method][0]):
            out_dir = os.path.join(args.data, args.out_dir, use, mm)
            os.mkdir(out_dir)
            lbl = LABLES[0] if mm == 'pristine' else LABLES[1]
            # max_frames = args.num_frames_train if use == USAGES[0] else args.num_frames_test_val
            ...
            for fn in splits_dict[use][lbl]:
                ...
                save_faces(video_path = video_path, max_frames = max_frames, json_bb_path = json_bb_path, out_face_dir = out_dir, fn_extension=fn_ext)
                pbar.update(1)
    pbar.close()
