...
def train(train_queue, model, criterion, optimizer):
    ...
    for step, (input, target) in enumerate(train_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('Train Step: %03d Objs: %e Top2: %f Top1: %f', step, objs.avg, top2.avg, top1.avg)
...
def infer(valid_queue, model, criterion):
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        if step % args.report_freq == 0:
            logging.info('Valid Step: %03d Objs: %e Top2: %f Top1: %f', step, objs.avg, top2.avg, top1.avg)
...
def get_data(args, batch_size):
    ...
    train_transform, val_transform  = utils._data_transform_mult(args, size=160, mean=mean, std=std)

    train_data = dset.ImageFolder(root=os.path.join(args.data_dir, 'train'), transform=train_transform)
    test_data = dset.ImageFolder(root=os.path.join(args.data_dir, 'val'), transform=val_transform)
