...
parser.add_argument('--data_dir_train', type=str, default='data', help='data dir train')
parser.add_argument('--data_dir_val', type=str, default='data', help='data dir val')
...
def get_data(args, batch_size):
    ...
    train_transform, val_transform = utils._data_transform_search(args, size=80, mean=mean, std=std)

    train_data = dset.ImageFolder(root=os.path.join(args.data_dir_train), transform=train_transform)
    val_data = dset.ImageFolder(root=os.path.join(args.data_dir_val), transform=val_transform)
