...
parser.add_argument('--test_freq', type=float, default=20, help='report frequency')
...
def main():
    ...
    # new_batch_size = infer_batch_size(args, args.batch_size)
    ...
    for sp in range(len(num_to_keep)):
        model = Network(args.init_channels + int(add_width[sp]), NUM_CLASESS, args.layers + int(add_layers[sp]), criterion, switches_normal=switches_normal, switches_reduce=switches_reduce, p=float(drop_rate[sp]))
        ...
        for epoch in range(epochs):
            ...
            if epochs - epoch < 5:
                valid_acc, valid_obj = infer(valid_queue, model, criterion, data_type='valid')
                logging.info('Valid_acc %f', valid_acc)
                logging.info('Valid_objs %f', valid_obj)
...
def infer(valid_queue, model, criterion, data_type='valid'):
    objs = utils.AvgrageMeter()
    top1 = utils.AvgrageMeter()
    model.eval()
    ...
    for step, (input, target) in enumerate(valid_queue):
        ...
        if step % args.report_freq == 0:
            logging.info(f'{data_type} %03d %e %f', step, objs.avg, top1.avg)
...
def get_data(args, batch_size):
    ...
    train_transform, val_transform = utils._data_transform_ff_search(args, size=40, mean=mean, std=std)
...
def infer_batch_size(args, batch_size):
    working_batch_size = False
    new_batch_size = copy.deepcopy(batch_size)
    while not working_batch_size or new_batch_size == 0:
        train_queue, valid_queue, test_queue = get_data(args,new_batch_size)
        # build Network
        criterion = nn.CrossEntropyLoss()
        criterion = criterion.cuda()
        switches = []
        ...
        if len(args.dropout_rate) == 3:
            ...
            drop_rate = [0.0, 0.0, 0.0]
        sp = 0
        model = Network(args.init_channels + int(add_width[sp]), NUM_CLASESS, args.layers + int(add_layers[sp]),
        ...
        scheduler.step()
        lr = scheduler.get_lr()[0]
        model.module.p = float(drop_rate[sp]) * np.exp(-(0 - 10) * scale_factor)
        model.module.update_p()
        ...
        for step, (input, target) in enumerate(train_queue):
            ...
            try:
                logits = model(input)
                logging.info("New BATCH_SIZE: %s", new_batch_size - 5)
                del model
                return new_batch_size - 5
            except:
                logging.info("NOT enough GPU memory with BATCH_SIZE: %s", new_batch_size)
                new_batch_size -= 1
                working_batch_size == False
                break
        del model
